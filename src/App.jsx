...
export default function App() {
  ...
  const sendDesktopNotification = (mode) => {
    if (Notification.permission === "granted") {
      const title = mode === 'work' ? "¡Tiempo de Enfoque Terminado!" : "¡Descanso Terminado!";
      const body = mode === 'work' ? "Buen trabajo. Tómate un descanso merecido." : "Es hora de volver a la acción.";
  ...
  const getUrgencyConfig = (days, isOverdue, isCompleted = false) => {
    if (isCompleted) {
      return { type: 'completed', bgColor: 'bg-slate-300', textColor: 'text-slate-500', icon: <CheckCircle2 size={18} />, borderColor: 'border-slate-300' };
    }
  ...
  const openCreateModal = () => {
    ...
    setNewTaskDate(localISODate); setNewTaskTime(''); setNewTaskTitle(''); setNewTaskTag(''); setIsModalOpen(true);
  };
  ...
  // --- CONTROLES FOCUS ---
  const startSession = () => {
    if (!focusState.activeTaskId && tasks.length > 0) {
      const firstTask = tasks.find(t => !t.completed);
      if (firstTask) setFocusState(prev => ({ ...prev, activeTaskId: firstTask.id }));
    }
    const wSecs = workMinutes * 60;
    const rSecs = (workMinutes / 4) * 60;
    setFocusState(prev => ({ ...prev, isSessionActive: true, workLeft: wSecs, restLeft: rSecs, initialWork: wSecs, initialRest: rSecs, mode: 'work', isRunning: true }));
  };
  const stopSession = () => setIsStopSessionModalOpen(true);
  const confirmStopSession = () => { setFocusState(prev => ({ ...prev, isSessionActive: false, isRunning: false, mode: 'work' })); setIsStopSessionModalOpen(false); };
  const switchMode = () => setFocusState(prev => ({ ...prev, mode: prev.mode === 'work' ? 'rest' : 'work', isRunning: true }));
  const toggleRun = () => setFocusState(prev => ({ ...prev, isRunning: !prev.isRunning }));
  ...
  const CalendarMonth = ({ date }) => {
    ...
    return (
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-3">
        <div className="text-center font-bold text-slate-700 dark:text-slate-200 mb-3">{capitalizedMonth} {year}</div>
        ...
        <div className="grid grid-cols-7 gap-1">
          {(() => {
            ...
            return [...blanks, ...days].map((day, index) => {
              ...
              let dayTextColor = "text-slate-600 dark:text-slate-400";
              ...
              return (
                ...
                >
                  ...
                  {dayTasks.length > 0 && (
                    <div className={`absolute bottom-1 w-1 h-1 md:w-1.5 md:h-1.5 rounded-full ${dotColor}`}></div>
                  )}
                </button>
              );
            });
          ...
          <div className="p-4">
            <div className={`flex items-center mb-8 text-indigo-600 dark:text-indigo-400 font-bold text-xl overflow-hidden whitespace-nowrap ${isSidebarCollapsed ? 'justify-center' : 'justify-between'}`}>
              <div className={`flex items-center gap-3 ${isSidebarCollapsed ? 'hidden' : 'flex'}`}>
                <div className="min-w-[32px] h-8 bg-indigo-600 rounded-lg flex items-center justify-center text-white"><CheckCircle2 size={20} strokeWidth={3} /></div>
                <span>Mi Gestor</span>
              </div>
              <button onClick={() => setIsSidebarCollapsed(!isSidebarCollapsed)} className="p-2 text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg transition-colors">
                {isSidebarCollapsed ? <PanelLeftOpen size={24} /> : <PanelLeftClose size={24} />}
              </button>
            </div>
            <nav className="space-y-2">
              <button onClick={() => setCurrentView('tasks')} title="Tareas" className={`w-full flex items-center gap-3 p-3 rounded-xl transition-all ${currentView === 'tasks' ? 'bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-300 shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700'} ${isSidebarCollapsed ? 'justify-center' : ''}`}>
                <ListTodo size={24} /><span className={`font-medium ${isSidebarCollapsed ? 'hidden' : 'block'}`}>Tareas</span>
              </button>
              <button onClick={() => setCurrentView('focus')} title="Enfoque" className={`w-full flex items-center gap-3 p-3 rounded-xl transition-all ${currentView === 'focus' ? 'bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-300 shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700'} ${isSidebarCollapsed ? 'justify-center' : ''}`}>
                <Timer size={24} /><span className={`font-medium ${isSidebarCollapsed ? 'hidden' : 'block'}`}>Enfoque</span>
              </button>
              <button onClick={() => setCurrentView('calendar')} title="Calendario" className={`w-full flex items-center gap-3 p-3 rounded-xl transition-all ${currentView === 'calendar' ? 'bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-300 shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700'} ${isSidebarCollapsed ? 'justify-center' : ''}`}>
                <CalendarIcon size={24} /><span className={`font-medium ${isSidebarCollapsed ? 'hidden' : 'block'}`}>Calendario</span>
              </button>
            </nav>
          </div>
          <div className="p-4 border-t border-slate-100 dark:border-slate-700">
            <button onClick={() => setIsDarkMode(!isDarkMode)} title={isDarkMode ? 'Modo Claro' : 'Modo Oscuro'} className={`w-full flex items-center gap-3 p-3 rounded-xl text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors ${isSidebarCollapsed ? 'justify-center' : 'justify-start'}`}>
              {isDarkMode ? <Sun size={22} /> : <Moon size={22} />}
              <span className={`font-medium ${isSidebarCollapsed ? 'hidden' : 'block'}`}>{isDarkMode ? 'Modo Claro' : 'Modo Oscuro'}</span>
            </button>
          </div>
        </aside>
            ...
            <div className="max-w-5xl mx-auto pb-24 animate-in fade-in duration-300 h-full flex flex-col">
              <header className="mb-6 flex flex-col md:flex-row items-center justify-between gap-4">
                <h1 className="text-3xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
                  <CalendarIcon className="text-indigo-600" /> Calendario
                </h1>

                {/* Controles de Navegación */}
                <div className="flex gap-2 bg-white dark:bg-slate-800 p-1 rounded-xl border border-slate-200 dark:border-slate-700">
                  <button onClick={() => navigateCalendar(-1)} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg">
                    <ChevronLeft size={20} className="text-slate-600 dark:text-slate-300" />
                  </button>
                  <div className="px-4 py-2 font-bold text-slate-700 dark:text-slate-200 text-center text-sm capitalize w-32">
                    {calendarCursor.toLocaleDateString('es-ES', { month: 'short', year: 'numeric' })}
                  </div>
                  <button onClick={() => navigateCalendar(1)} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg">
              ...
              {/* GRID CALENDARIO DUAL */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                <CalendarMonth date={calendarCursor} />
                <CalendarMonth date={new Date(calendarCursor.getFullYear(), calendarCursor.getMonth() + 1, 1)} />
              </div>

              {/* LISTA DE TAREAS SELECCIONADA */}
              <div className={`bg-slate-50 dark:bg-slate-800/30 rounded-2xl p-5 border border-slate-200 dark:border-slate-700 flex-grow transition-all duration-300 ${selectedDate ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4 pointer-events-none'}`}>
                {selectedDate && (
                  <>
                    <h3 className="text-lg font-bold text-slate-800 dark:text-white mb-4 flex items-center gap-2">
                      <span>{new Date(selectedDate + 'T00:00:00').toLocaleDateString('es-ES', { day: 'numeric', month: 'long' })}</span>
                      <span className="text-xs font-normal bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-300 px-2 py-0.5 rounded-full">{tasks.filter(t => t.date === selectedDate).length} tareas</span>
                    </h3>

                    <div className="space-y-2 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
                      {tasks.filter(t => t.date === selectedDate).length === 0 && (
                        <div className="text-center py-8 text-slate-400 italic">No hay tareas para este día.</div>
                      )}
